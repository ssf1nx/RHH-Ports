...
- Download the decode tool and run it:
   ```bash
   curl -L -o yyg_qoi_decode.exe https://github.com/JeodC/RHH-Ports/raw/main/ports/released/gamemakerengine/grappledog/grappledog/tools/yyg_qoi_decode.exe
   yyg_qoi_decode.exe data.win textures --repack --page-size 1024 --block 6x6 --quality -fastest --set-flags Fullscreen --clear-flags Scale
   ```
...
Linux / macOS users can build `yyg_qoi_decode` from source in [buildtools/yyg_qoi_decode/](https://github.com/JeodC/RHH-Ports/tree/main/buildtools/yyg_qoi_decode) (`cmake -B build && cmake --build build`).
